...
function DNSServer(backend, opts) {
    this.opts = opts || optsDefaults;

    this.server = dns.createServer();
    this.backend = backend;

    if(this.opts.upstream.enabled) {
        this.dnsClient = new DNSClient(this.opts.upstream.ip, this.opts.upstream.client);
    }

    this.server.on('request', this.requestHandler.bind(this));
    this.server.on('error', this.errorHandler.bind(this));

    // enable the simulated round-robin strategy by default
    this.strategy = strategy.RoundRobin;
}
...
// Generates a DNS response object for the native-dns factory
